...
func process_targets(damage_types) -> void:
	target = collider_ref #just the thing hit
	apply_damage(target,damage_types)

func process_damage(ball: Ball, collider: Node2D, damage_types) -> void: # entry point--should not be overriden
	ball_ref = ball
	collider_ref = collider
	process_targets(damage_types)

func apply_damage(damage_target: Node2D, damage_types) -> void: #note i am passing damage types a few times because i want to deal with brick and enemy reactions on those scenes
